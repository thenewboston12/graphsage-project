...
class EncoderSC(nn.Module):
    ...
    def __init__(self, features, feature_dim, 
        ...
        # Extra projection layer for skip connection if dimensions don't match
    ...
    def forward(self, nodes):
        ...
        # Core difference: apply skip connection
        raw = combined  # Save original
        combined = self.weight.mm(combined.t())  # Main transformation

        if self.skip_proj is not None:
            raw = self.skip_proj(raw)

        combined = combined + raw.t()  # Skip connection addition
